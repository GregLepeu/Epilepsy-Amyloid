...
desired_syndromes = ['Language', 'Amnestic', 'Visual-Spatial', 'Diffuse']
df  = df[df['Syndrome'].isin(desired_syndromes)]

# Step 1: Group the data by 'group' and 'ATN status' and count occurrences
group_ATN_counts = df.groupby(['Epi_ND', 'AT status']).size().reset_index(name='counts')


# Step 3: Create a contingency table
contingency_table = group_ATN_counts.pivot(index='AT status', columns='Epi_ND', values='counts').fillna(0)
...
for i, group in enumerate(unique_groups):
    ...
    filtered_data['AT status'] = pd.Categorical(filtered_data['AT status'], categories=order, ordered=True)
    filtered_data = filtered_data.sort_values('AT status')

    # Pie chart labels and sizes
    labels = filtered_data['AT status']
